...
class SimpleRxnRate {
public:
    SimpleRxnRate(const sunrealtype kf, const sunrealtype kb, const sunrealtype tol = 1.e-12) : kf(kf), kb(kb), tol(tol) {}


    sunrealtype forward(const N_Vector y, 
                        const std::array<sunindextype,n_reactants> & indices, 
    ...
    std::array<sunrealtype, n_reactants> forward_derivatives(const N_Vector y, 
                             const std::array<sunindextype,n_reactants> & indices, 
                            const std::array<sunrealtype,n_reactants> & orders) const{
    ...
    sunrealtype backward(const N_Vector y, 
                        const std::array<sunindextype,n_products> & indices, 
    ...
    std::array<sunrealtype, n_products> backward_derivatives(
        const N_Vector y,
        const std::array<sunindextype, n_products> & indices,
        const std::array<sunrealtype, n_products> & orders
    ) const {
...
int main(){
    ...
    std::function<int(sunrealtype , N_Vector , N_Vector)> rhs 
        = [=](sunrealtype t, N_Vector y, N_Vector ydot){
    ...
    jac = [=](sunrealtype t, N_Vector y, N_Vector ydot, SUNMatrix J, 
